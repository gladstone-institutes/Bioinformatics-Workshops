---
title: "Machine Learning Using R - Part 2"
...
**Note:** K-means requires numeric input, so we use `cough_severity_encoded` (numeric) rather than `cough_severity` (categorical).

## Check for Redundant Features

K-means uses distance between points, so if two features carry nearly the same information, that information is effectively **counted twice** and gets extra weight in the clusters. Checking for redundant features before clustering can improve clustering quality.

The simplest check is `cor()`. Correlation ignores scale, so we can run it before standardizing:

```{r check-redundancy}
# Correlation between every pair of features
cor_matrix <- cor(cluster_data)

# List feature pairs that are highly correlated (|r| > 0.7)
as.data.frame(as.table(cor_matrix)) %>%
  rename(feature_1 = Var1, feature_2 = Var2, correlation = Freq) %>%
  filter(as.character(feature_1) < as.character(feature_2)) %>%  # keep each pair once
  filter(abs(correlation) > 0.7) %>%
  arrange(desc(abs(correlation)))
```

What we see:  
- **`cough` and `cough_severity_encoded`**: both describe whether (and how badly) a patient coughs  
- **`rhonchi` and `wheezes`**: both are abnormal breath sounds heard during a lung exam, so they often occur together  

We'll keep all features for this demo, but for your own data you could drop or combine one feature from each highly correlated pair and see how the clusters change.

**Going further:** The **variance inflation factor (VIF)** is a more formal check that measures how well each feature can be predicted from *all the others* (not just one at a time). See `car::vif()`. A common rule of thumb is that a VIF above 5-10 signals a redundant feature. This matters most for K-means and linear models; tree-based methods like XGBoost are much less affected.
...
In **Part 3**, we'll explore **XGBoost** - a supervised learning method for classification!
