...
By default, the class with probability > 0.5 is selected. You can adjust this threshold based on your needs (e.g., lower threshold to catch more positives):

```{r custom-threshold}
# Default threshold is 0.5
# Lower threshold = more sensitive (catch more positives, but more false alarms)
# Higher threshold = more specific (fewer false alarms, but may miss positives)

threshold <- 0.3  # Example: classify as Positive if probability > 0.3

rf_predictions_custom <- ifelse(rf_proba$Positive > threshold, "Positive", "Negative")
rf_predictions_custom <- factor(rf_predictions_custom, levels = c("Negative", "Positive"))

# Compare default vs custom threshold
cat("Default (0.5) - Positive predictions:", sum(rf_predictions == "Positive"), "\n",
    "Custom  (0.3) - Positive predictions:", sum(rf_predictions_custom == "Positive"), "\n")
```
