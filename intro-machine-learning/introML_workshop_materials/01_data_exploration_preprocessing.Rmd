...
# For comprehensive data summaries
library(skimr)
```
...
```
```{r explore-data}
# Comprehensive overview with skimr
# This shows: dimensions, column types, summary stats, and missing values
skim(covid_data)
```

**Key observations from above:**  
- **n_missing column**: Shows how many missing values each variable has  
- **complete_rate**: Proportion of non-missing values (1.0 = no missing data)  
...
Some algorithms (Random Forest, SVM, K-means) require numeric input, so we need to encode categorical variables.

```{r label-encoding}
# Encode ordinal cough_severity: None=0 < Mild=1 < Moderate=2 < Severe=3
