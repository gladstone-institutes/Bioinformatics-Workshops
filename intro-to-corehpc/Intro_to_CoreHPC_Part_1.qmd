...
Which of these are you already comfortable with? Select all that apply.

1.  The Unix command line (`cd`, `ls`, editing a file)
2.  Writing scripts or code (any language)
3.  Working on an HPC cluster
4.  Containers (Docker, Apptainer)
5.  None of these yet
...
![](slide_materials/CoreHPC_diagram.svg){fig-alt="CoreHPC architecture diagram showing bastion nodes, login node, compute nodes, and Globus endpoints" .nostretch fig-align="center" width=60%}
...
-   Both run the **same software stack** (Linux, OpenHPC modules, Apptainer, SLURM)
...
| Persistent storage | FAC Storage | HIVE |
...
-   You must be connected to either the Gladstone or UCSF networks or VPN for these steps to work
...
-   **Submit jobs** and **stage data**, not for heavy compute
...
-   Also have fast **local NVMe** at `/tmp` that lasts only for the job
    -   This is a per-job `/mnt/lscratch/<job id>` mounted as `/tmp`


## Hands-on: Log In and Look Around {.smaller}

1.  SSH to the **bastion** for your half, then hop to its **login node**
2.  Run `hostname`, `whoami`, and `pwd`

```{r, engine='bash', eval=TRUE, results='markup', comment=NA, highlight=TRUE, echo=FALSE}
echo '[alice@chpc-gs-login-vm1 ~]$ hostname
chpc-gs-login-vm1
[alice@chpc-gs-login-vm1 ~]$ whoami
alice
[alice@chpc-gs-login-vm1 ~]$ pwd
/home/remote/alice'
```

3.  To close the connection run the `exit` command **twice**


## Getting an Interactive Session {.smaller}

CoreHPC has **no dev nodes**, so you ask SLURM for a compute node instead:

```bash
salloc --partition=cpu --cpus-per-task=1 --mem=4G --time=01:00:00
```

-   `--partition=cpu` picks the pool of general CPU nodes
-   The rest is **how much** and **how long**, covered in Part 2
-   You get a shell on the node once the request is granted
-   Type `exit` to **release** the node as soon as you are done
    -   Otherwise it stays reserved for you until `--time` runs out

## Why Use an Interactive Session {.smaller}

-   **Test before you submit**, so a typo costs you seconds instead of a queued job
-   **Work interactively** in R or Python against real data
-   **Build software** you downloaded on the login node
    -   Compute nodes have no internet, so download first, compile second
-   **Explore big files**, since sorting a 50 GB BAM on the login node slows it for everyone
-   **Debug a job that failed**, using the same resources it asked for

## Hands-on: Get a Compute Node {.smaller}

1.  From the login node, request a small interactive session
2.  Run `hostname` again and note that you are on a **different machine**

```{r, engine='bash', eval=TRUE, results='markup', comment=NA, highlight=TRUE, echo=FALSE}
echo '[alice@chpc-gs-login-vm1 ~]$ salloc --partition=cpu --cpus-per-task=1 --mem=4G --time=00:30:00
salloc: Granted job allocation 152334
[alice@chpc-gs-cpu-01 ~]$ hostname
chpc-gs-cpu-01'
```

3.  Type `exit` to release the node before we move on
...
The paths are the **same on both halves**, except for persistent storage:
...
| Local scratch | `/tmp` (inside a job only) |
| Persistent | FAC share (UCSF) or `/mnt/gladstone/<pi>/<share>` (Gladstone) |
...
-   Compute nodes also have even faster **node local** scratch
    -   Inside a job, write to **`/tmp`**
    -   SLURM gives each job its own `/mnt/lscratch/<job id>`, mounted as `/tmp`
    -   It is deleted when the job ends, so copy anything you need out
...
-   Avoid reading/writing large directories of tiny files on shared storage when possible
    -   It slows the filesystem for **everyone**, not just you

## Poll 1

Your job results have sat in `/mnt/scratch/user/$USER` for three weeks. Which is true?

1.  Safe, scratch is backed up nightly
2.  Safe, support can restore them after the purge
3.  Possibly gone, and unrecoverable
4.  Already copied to your home directory
...
-   **Globus** is the supported way to move data in or out from outside CoreHPC
    ...
    -   This covers your laptop and other institutions (UCSF FAC <-> Gladstone HIVE)
...
module load CBI miniforge3  # load a module
...
-   Available as a module: `module load CBI miniforge3`
...
Your conda env lives in `/mnt/scratch/user/$USER/envs`. What do you keep elsewhere?

1.  A copy of the env directory
2.  The `environment.yml` recipe
3.  Nothing, conda envs are rebuilt automatically
4.  The conda package cache
...
-   One `.sif` file is more efficient on shared storage than a conda env's **millions of tiny files**
...
-   Full command reference: the [Apptainer user guide](https://apptainer.org/docs/user/main/)
...
You need to download a container image. Which node do you do it on?

1.  The bastion
2.  The login node
3.  A compute node
4.  Any of them, they share a filesystem
