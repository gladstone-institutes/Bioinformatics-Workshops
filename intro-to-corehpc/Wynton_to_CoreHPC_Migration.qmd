...
# -> at the Duo prompt type 1 (push is the only option), then approve on your phone
...
| `/wynton/scratch`           | `/mnt/scratch/user/$USER` (shared)                     |
| node-local `/scratch`       | `/tmp` (per-job `/mnt/lscratch/<job id>`)              |

::: {.bottom-message}
A `/scratch` symlink exists for now, but it may be removed: write `/mnt/scratch`.
:::
...
| `-l scratch=2G`               | no flag; write to local `/tmp`                   |
