...
# salloc session.
#
# IMPORTANT: your script must read the size from the BATCH_SIZE env var, exit 0
# on success, and exit nonzero on OOM. An uncaught CUDA OOM exception already
# does this, just don't catch it and exit 0:
#   import os
...
# ----------------------------------------------------------------------------
PYTHON_SCRIPT=train.py   # name of your training script (use `srun python` below if your salloc shell is on the submit node)
START_BATCH_SIZE=1024    # largest size to try, rounded down to a multiple of STEP; set it above any size that might fit
MIN_BATCH_SIZE=8         # smallest size to try, rounded down to a multiple of STEP (floor: STEP); if even this OOMs, nothing fit
...
while [ "$low" -le "$high" ]; do
  ...
  if [ "$exit_code" -eq 0 ]; then
    ...
    best=$candidate
    low=$(( candidate + STEP ))
  # 137 = SIGKILL (host OOM killer); a CUDA OOM exiting 1 is caught by the log grep.
