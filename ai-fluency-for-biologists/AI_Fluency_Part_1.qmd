---
title: "AI Fluency for Biologists"
subtitle: "Part 1"
author: "Natalie Gill"
institute: "Gladstone Bioinformatics Core"
date: "April, 20, 2026"
date-format: MMMM DD, YYYY
format:
  revealjs:
    theme: [default, ai-theme.scss]
    highlight-style: arrow
    code-line-numbers: false
    template-partials:
      - title-slide.html
---

```{r}
#| label: setup
#| include: false
library(countdown)
```

##

<center>*Press the ? key for tips on navigating these slides*</center>

## Introductions

**Natalie Gill**   
Bioinformatician II  


## Poll 1

**What is your level of experience with AI tools like ChatGPT or Claude?**

1. I've never used them
2. I've tried them a few times
3. I use them regularly
4. I use them daily in my work

## Target Audience

- No prior AI or programming experience required
- Designed for biologists at all career stages
- Laptop and a free Claude account are all you need

## Learning Objectives

1. Identify tasks that are well-suited for AI assistance
2. Write clear, effective prompts to get useful results
3. Evaluate AI outputs for accuracy and appropriateness
4. Recognize limitations and potential biases in AI responses
5. Apply ethical best practices when using AI in research

## Part 1:

1. Introduction to AI tools and the 4 Ds framework
2. Module 1: Delegation
3. Break
4. Module 2: Description


# Introduction

## What is AI Fluency?

The ability to work with AI systems in ways that are **effective**, **efficient**, **ethical**, and **safe**

- Not about becoming an AI expert
- About developing practical skills to use AI as a tool in your research
- Like learning to use a new instrument - you need to know its strengths and limitations

## The 4 Ds Framework

A practical approach to working with AI effectively:

1. **Delegation** - Deciding *what* to ask AI to do
2. **Description** - Crafting *how* you ask (prompting)
3. **Discernment** - Evaluating *whether* the output is correct
4. **Diligence** - Using AI *responsibly* and ethically


## {.center}

![](assets/four_ds_diagram.svg){fig-alt="The 4 Ds of AI Fluency: Delegation, Description, Discernment, and Diligence arranged in an interconnected diamond with bidirectional arrows between each node" fig-align="center" width=100%}

## AI as Augmentation, Not Replacement

AI tools are **thinking partners**, not replacements for your expertise

- Your domain knowledge is essential - AI doesn't know your system, your data, or your scientific question
- AI is useful for tasks like brainstorming, summarizing, drafting, and troubleshooting
- You remain responsible for the scientific validity of your work

<!-- 

Note: This is probably well covered by the other slides
## Current AI Landscape {.smaller}

**Where AI can help in biology:**

- Literature search and summarization
- Writing and editing manuscripts, grants, and protocols
- Designing experiments and analysis strategies
- Explaining unfamiliar concepts or statistical methods

**Where caution is needed:**

- AI can "hallucinate" - generating plausible but fabricated references, gene names, or statistics
- Outputs may reflect biases in training data
- Sensitive or unpublished data should not be shared with AI tools without considering privacy policies
- AI-generated figures should not be used in publications - they can misrepresent data and many journals prohibit them -->

## Brief Overview of Tools

::: {style="font-size: 0.8em;"}
Popular tools include **Claude** (Anthropic), **ChatGPT** (OpenAI), and **Gemini** (Google)

- All are **large language models (LLMs)** - trained on vast text data to understand and generate language
- They share the same core limitations: hallucinations, knowledge cutoffs, and training biases
- Relative performance varies by task and changes frequently
- They do not search the internet by default (some have optional web search)
:::

## Choosing a Model {.smaller}

<!-- TODO: Slide on factors that affect model selection for end users
Guidance:
- Cost vs. capability tradeoff (free tiers vs. paid plans)
- Task fit: some models are better at coding, some at writing, some at reasoning
- Context window size — how much text you can provide at once
- Multimodal capabilities (image/PDF input, etc.)
- Privacy and data policies vary by provider and plan tier
- Speed vs. quality: smaller/faster models for simple tasks, larger models for complex reasoning
- For most biologists: pick one good tool and learn it well rather than switching constantly
-->

## Understanding Benchmarks and Leaderboards {.smaller}

<!-- TODO: Slide on how to interpret AI benchmark leaderboards
Guidance:
- Mention common leaderboards: Chatbot Arena (LMSYS), HELM, benchmarks on model provider websites
- Key benchmarks to know: MMLU (general knowledge), HumanEval (coding), GPQA (graduate-level science)
- Caveats for end users:
  - Benchmarks measure narrow tasks — high scores don't guarantee good performance on YOUR task
  - Leaderboard rankings change frequently; don't over-optimize model choice based on them
  - "Vibes-based" evaluation (Chatbot Arena) vs. standardized benchmarks — both have value
  - Contamination: models may have seen benchmark questions in training data
  - The best approach: try your actual task on 2-3 models and see which works best for you
- Main takeaway: benchmarks are useful for rough comparisons, but your own experience matters more
-->

## What Are AI Agents? {.smaller}

<!-- TODO: Slide introducing the concept of AI agents
Guidance:
- An agent is an AI system that can take actions autonomously — not just generate text
- Key capabilities that distinguish agents from basic chat:
  - Tool use: agents can search the web, run code, read files, call APIs
  - Multi-step planning: agents break down a goal into steps and execute them
  - Iteration: agents can check their own work and retry if something fails
- Examples relevant to biologists:
  - Research mode / deep research: agent searches multiple sources and synthesizes findings
  - Code agents: write, run, and debug analysis scripts autonomously
  - Computer use: agents that can interact with software interfaces
- This connects to the Agency modality from the Delegation module
- Current limitations: agents can go off-track, may take unexpected actions, still need human oversight
- Practical advice: start with augmentation mode, graduate to agents as you build trust
-->



# Module 1: Delegation

## Goal and Task Awareness

::: {style="font-size: 0.8em;"}
Before opening any AI tool, clearly define what you need:

- **What is the task?** e.g., summarize a paper, draft a methods section, troubleshoot a protocol
- **What does success look like?** Define the output you want before you start
- **Do you have the expertise to evaluate the result?** If you can't judge whether the AI is right, you probably shouldn't delegate it
- Together with **Platform Awareness**, this helps you decide if AI is the right tool for the job
:::

## Platform Awareness

::: {style="font-size: 0.8em;"}
Understanding what AI tools can and cannot do:

- **Context window** -- how much text AI can process at once. Exceeding it causes the AI to "forget" earlier content
- **Knowledge cutoff** -- AI has no knowledge after its training date. It won't know recent papers
- **Hallucination** -- AI can confidently state plausible but wrong things (fake citations, invented gene names)
- **Context pollution** -- long conversations accumulate errors; start a new chat for new tasks
- **Extended thinking / research modes** -- newer features for complex reasoning or searching external sources
:::


## Choosing an AI Tool

**Before choosing a tool, consider the privacy policy:**

- What data is stored? Is it used to train future models?
- Does your institution have approved tools or data use agreements?
- Never share sensitive, unpublished, or patient-derived data without checking your institution's policies

## {.center}

![](assets/task_delegation_modalities.svg){fig-alt="Three modalities of task delegation shown as cards: Automation (you define, AI executes), Augmentation (you and AI collaborate as thinking partners), and Agency (you configure, AI acts independently), with bar charts showing relative human versus AI involvement" fig-align="center" width=95%}

## Modalities of Task Delegation

::: {style="font-size: 0.7em;"}
- **Automation** -- You define exactly what needs to be done, and the AI executes it

:::{.example-indented}
"Rewrite this protocol section to follow the journal's formatting guidelines, do not change the content - just the formatting."
:::

- **Augmentation** -- You and AI collaborate iteratively as thinking partners

:::{.example-indented}
Drafting and refining an abstract together over several rounds of feedback
:::

- **Agency** -- You configure the AI's role and knowledge, then let it act independently

:::{.example-indented}
Giving AI your experimental goals and reagent constraints, and letting it independently design and compare multiple protocol options
:::

In practice, **Augmentation** is the most productive mode for research tasks
:::

## Exercise: Identify Your AI Tasks

::: {style="font-size: 0.8em;"}
**Think about your own work and identify 2-3 tasks where AI could help.**

For each task, consider:

1. What is the task? (e.g., literature review, writing, data wrangling)
2. Which mode fits best -- Automation, Augmentation, or Agency?
3. Could you evaluate whether the AI's output is correct?

Enter tasks [here](https://app.sli.do/event/3sDxDAGKp3d6j1LfuKN4Si).
:::


# 10 min break

<center>

```{r}
#| echo: false
countdown::countdown(minutes = 10,
                     seconds = 0,
                     color_border = "black",
                     color_running_background = "#47d193",
                     color_finished_background = "#a3184e",
                     padding = "50px",
                     margin = "5%",
                     font_size = "4em",
       style = "position: relative; width: min-content;")
```

</center>


# Module 2: Description

## Prompting Techniques Overview

<!-- TODO: Revise this list based on current best practices (see guidance below)

WHAT TO KEEP (still strongly recommended):
1. Provide Context — still the #1 technique across all providers. Keep as-is.
2. Show Examples (few-shot) — still highly effective. Research shows it can improve
   accuracy from 0% to 90% on structured tasks. Keep as-is.
3. Specify Output Constraints — still essential. Keep as-is.
4. Break Complex Tasks into Steps — still valuable for complex tasks. Keep as-is.

WHAT TO UPDATE:
5. "Ask It to Think First" — REVISE. Modern models (Claude, GPT-5, Gemini 3) have
   built-in reasoning modes. You no longer need to say "think step by step" — just
   toggle on extended thinking / reasoning mode in the UI. The slide already says this
   but could be stronger: emphasize that explicit CoT prompting is now unnecessary
   for frontier models and can actually hurt performance (adds latency for minimal
   gain, per Wharton 2025 research).

WHAT TO DEMOTE OR REMOVE:
6. "Define the AI's Role" — DEMOTE. Research (Schulhoff / The Prompt Report) shows
   role prompting is "largely ineffective" for correctness. It may slightly influence
   tone but doesn't improve accuracy. Consider moving it to a "minor tips" section
   or removing it as a primary technique.

WHAT TO ADD:
- "Be Direct and Specific" — The current consensus #1 technique. Tell the AI exactly
  what you want rather than hinting. Avoid vague instructions. Anthropic's framing:
  "Think of the AI as a brilliant but new employee who lacks context."
- "Iterate and Refine" — Already in the exercise but deserves its own technique slot.
  Conversation is iterative — your first prompt rarely needs to be perfect. Follow up
  with corrections, clarifications, and refinements.
- "Use the AI to Improve Your Prompt" (meta-prompting) — Already exists as a "Bonus"
  slide. Consider promoting it since all providers now recommend this approach.

SUGGESTED REVISED LIST:
1. Be Clear and Specific (new — the fundamental principle)
2. Provide Context (keep)
3. Show Examples (keep)
4. Specify Output Constraints (keep)
5. Break Complex Tasks into Steps (keep)
6. Iterate and Refine (promoted from exercise)
Bonus: Use extended thinking for hard problems (simplified from current #5)
Bonus: Meta-prompting (already exists)

ADDITIONAL NOTES:
- The "Bad Prompt vs Good Prompt" slide is excellent — keep it
- Consider adding: "Your prompt style influences the output style" — if you write
  in bullets, the AI responds in bullets. If you write in prose, it responds in prose.
- The current slides are well-written; the main change is swapping role prompting
  for directness/iteration as primary techniques.
-->

::: {style="font-size: 0.8em;"}
1. **Provide Context** -- Give background information the AI needs
2. **Show Examples** -- Demonstrate the format or style you want
3. **Specify Output Constraints** -- Define length, format, audience, and structure
4. **Break Complex Tasks into Steps** -- Provide sequential instructions
5. **Ask It to Think First** -- Use extended thinking modes
6. **Define the AI's Role** -- Set perspective

These techniques map to the Description competency: Product, Process, and Performance Description
:::

## 1) Provide Context

::: {style="font-size: 0.8em;"}
Give the AI the background information it needs to do a good job.

- What field or subfield are you working in?
- What is the purpose of this task?
- What has already been done?


:::{.bad-example}
"Summarize this paper"
:::

:::{.good-example}
"I'm a graduate student in cardiac biology preparing for a journal club. Summarize this paper's main findings, focusing on the signaling pathways involved in cardiomyocyte differentiation."
:::
:::

## 2) Show Examples

Show the AI what you want by providing one or more examples of the desired output.

- Useful for defining **format**, **style**, or **level of detail**
- Also called "few-shot prompting" -- the AI learns from the pattern you demonstrate


:::{.example}
"Summarize each paper in this format:

[Author et al., Year] -- [One-sentence finding]. Method: [Key technique]. Relevance: [Why it matters to my project]."
:::

## 3) Specify Output Constraints

Tell the AI exactly what the output should look like:

- **Length** -- "in 200 words," "in 3 bullet points"
- **Format** -- "as a table," "as a numbered list," "in paragraph form"
- **Audience** -- "for a PI," "for a first-year graduate student," "for a grant review panel"
- **Structure** -- "organize by: (1) hypothesis, (2) methods, (3) key results"

The more specific your constraints, the closer the output will match what you need

## Bad Prompt vs. Good Prompt

::: {style="font-size: 0.8em;"}
:::{.bad-example}
"Summarize this paper"
:::

:::{.good-example}
"Summarize this cell biology paper's experimental methodology in exactly 200 words, focusing only on protein-protein interaction techniques (e.g., co-IP, pull-downs, FRET, crosslinking). Organize your summary by:

1. Primary techniques used
2. Key reagents
3. Validation methods

Write for an audience of post-docs and graduate students."
:::

The good prompt uses **context**, **output constraints**, and **structure** to guide the AI
:::

## 4) Break Complex Tasks into Steps

::: {style="font-size: 0.8em;"}
For complex tasks, give the AI a step-by-step process to follow.


:::{.example}
"Analyze this paper in the following steps:

Step 1: Identify the main hypothesis

Step 2: List the experimental methods used

Step 3: Summarize the key findings

Step 4: Evaluate whether the conclusions are supported by the data"
:::

This is called **Process Description** -- defining *how* the AI should approach the task
:::

## 5) Let It Think Deeply

::: {style="font-size: 0.8em;"}
Most AI tools now have a **thinking** or **reasoning** mode — use it for complex or multi-part questions

- **Don't** need to say "think step by step"
- **Do** toggle on extended thinking for harder questions; skip it for simple tasks

:::{.example}
*[Turn on extended thinking / reasoning mode]*

"Here is a paper on CRISPR-based gene therapy for sickle cell disease. What are the key methodological limitations, and how might they affect the authors' conclusions?"
:::
:::

## 6) Define the AI's Role

::: {style="font-size: 0.8em;"}
Set the AI's perspective to shape its responses.


:::{.example}
- "Explain this as a journal club presenter would to a mixed audience of biologists"
- "Write as if you are teaching a graduate student new to this subfield"
- "Act as a critical reviewer evaluating this manuscript for a high-impact journal"
:::

This is **Performance Description** -- defining how the AI should behave during your interaction (concise vs. detailed, supportive vs. critical)
:::

## Bonus: Ask AI to Help with Your Prompt

::: {style="font-size: 0.8em;"}
You can ask the AI to help you write a better prompt -- this is called **meta-prompting**.


:::{.example}
"I'm trying to analyze this paper's methodology for my literature review. Can you help me craft a better prompt to get a useful summary?"
:::

This is especially useful when:

- You're not sure how to structure your request
- You want to explore what's possible before committing to a specific approach
- Your initial prompt isn't giving you good results
:::

## Exercise: Prompting

::: {style="font-size: 0.8em;"}
**Use the task you identified in the Delegation exercise and write a prompt using at least 3 of the 6 techniques.**

**Steps:**

1. **Draft your prompt** using at least 3 techniques (3 min)
2. **Run it in Claude** and review the output (3 min)
3. **Iterate** -- refine your prompt and try again if needed (2 min)
4. **Reflect:** Which techniques helped most? What would you change? (2 min)


*Tip: Start with context + constraints + role, then add steps or examples if needed.*
:::


# End of Part 1


## Schedule for Part 2

1. Module 3: Discernment - Evaluating AI outputs
2. Module 4: Diligence - Responsible AI use
3. Practical application exercise
4. Q&A and discussion
5. Wrap-up and resources

## Workshop survey
- Please fill out our workshop survey so we can continue to improve these workshops

## Upcoming Workshops

::: {style="font-size: 0.8em;"}
*Details coming soon -- check our website for the latest schedule*
:::

## Acknowledgments

This workshop adapts the AI Fluency framework developed by Rick Dakan, Joseph Feller, and Anthropic (2025). Released under the [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/) license.

We thank Anthropic for providing temporary Claude account upgrades for workshop attendees and for the AI Fluency framework that forms the foundation of this workshop.
